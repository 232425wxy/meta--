...
func publish(s *Server, ctx context.Context, msg interface{}, t *testing.T, published chan struct{}, events ...map[string][]string) {
	if len(events) > 0 {
		err := s.PublishWithEvents(ctx, msg, events[0])
		assert.Nil(t, err)
...
func TestSubscribeMatches(t *testing.T) {
	...
	go publish(s, ctx, "hello", t, published, map[string][]string{"block.height": {"11"}})
...
func TestSubscribeNotMatches(t *testing.T) {
	...
	// 服务端发送消息
	published := make(chan struct{})
	go publish(s, ctx, "hello", t, published, map[string][]string{"block.height": {"10"}})
	published2 := make(chan struct{})
	go publish(s, ctx, "hello", t, published2)
...
func TestManyClientsSubscribe(t *testing.T) {
	...
	subscription1, err := s.Subscribe(ctx, clientID1, q1, 10)
	assert.Nil(t, err)
	subscription2, err := s.Subscribe(ctx, clientID2, q2, 20)
	assert.Nil(t, err)
	subscription3, err := s.Subscribe(ctx, clientID3, q3, 30)
	assert.Nil(t, err)
	...
	go publish(s, ctx, "client-01 can propose proposal", t, published1)
	go publish(s, ctx, "client-02 cannot propose proposal", t, published2, map[string][]string{"clientID2.power": {"19"}})
	go publish(s, ctx, "client-03 can propose proposal", t, published3)
...
func TestUnsubscribe(t *testing.T) {
	...
	go publish(s, ctx, "client-01 can propose proposal", t, published)
	go outputReceived(clientID, subscription, received)
...
func TestUnsubscribeNum(t *testing.T) {
	...
	subscription11, err := s.Subscribe(ctx, clientID1, q11)
	assert.Nil(t, err)
	subscription12, err := s.Subscribe(ctx, clientID1, q12)
	assert.Nil(t, err)
	subscription13, err := s.Subscribe(ctx, clientID1, q13)
	assert.Nil(t, err)
	subscription21, err := s.Subscribe(ctx, clientID2, q21)
	assert.Nil(t, err)
	subscription22, err := s.Subscribe(ctx, clientID2, q12)
	assert.Nil(t, err)
	...
	err = s.Unsubscribe(ctx, clientID1, q11)
	assert.Nil(t, err)
	assert.Equal(t, 2, s.NumClients())
	assert.Equal(t, 2, s.NumClientSubscriptions(clientID1))

	err = s.Unsubscribe(ctx, clientID1, q12)
	assert.Nil(t, err)
	assert.Equal(t, 2, s.NumClients())
	assert.Equal(t, 1, s.NumClientSubscriptions(clientID1))

	err = s.Unsubscribe(ctx, clientID1, q13)
	assert.Nil(t, err)
	assert.Equal(t, 1, s.NumClients())
	assert.Equal(t, 0, s.NumClientSubscriptions(clientID1))

	err = s.PublishWithEvents(ctx, "you can propose a new block", map[string][]string{"client-02.power": {"12.01"}})
	assert.Nil(t, err)
	assert.Equal(t, 1, s.NumClients())
	assert.Equal(t, 2, s.NumClientSubscriptions(clientID2))

	err = s.Unsubscribe(ctx, clientID2, q21)
	assert.Equal(t, 1, s.NumClients())
	...
	err = s.Unsubscribe(ctx, clientID2, q12)
...
func BenchmarkSubscribeMany(b *testing.B) {
	...
	for i := 0; i < 100; i++ {
		...
		subscription, err := s.Subscribe(ctx, client, q)
		assert.Nil(b, err)
		go func() {
			for {
				select {
	...
	for i := 0; i < b.N; i++ {
		id := i % 100
		err = s.PublishWithEvents(ctx, "message", map[string][]string{fmt.Sprintf("client-%d.power", id): {fmt.Sprintf("%d", i)}})
		assert.Nil(b, err)
	}
}
